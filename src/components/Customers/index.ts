...
export async function findAll(req: RequestWithUser, res: Response, next: NextFunction): Promise<void> {
    try {
        ...
        const customers: ICustomersModel[] = await CustomersService.findAll(pageNo,pageSize);
        const totalCount = await CustomersService.countAll();
        res.status(200).json({ totalCount: totalCount, records: customers, pageNo: pageNo+1, pageSize: pageSize });
    } catch (error) {
...
export async function search(req: RequestWithUser, res: Response, next: NextFunction): Promise<void> {
    try {
        const pageNo = req.query.pageNo ? parseInt(req.query.pageNo.toString()) - 1 : 0;
        const pageSize = req.query.pageSize ? parseInt(req.query.pageSize.toString()) : 20;
        const customers: ICustomersModel[] = await CustomersService.search(req.body, pageNo, pageSize);
        const totalCount = await CustomersService.countSearch(req.body);
        res.status(201).json({ totalCount: totalCount, records: customers, pageNo: pageNo, pageSize: pageSize });
    } catch (error) {
        next(new HttpError(error.message.status, error.message));
    }
}
...
export async function findOne(req: RequestWithUser, res: Response, next: NextFunction): Promise<void> {
    try {
        const customers: ICustomersModel = await CustomersService.findOne(req.params.id);
...
export async function create(req: RequestWithUser, res: Response, next: NextFunction): Promise<void> {
    try {
        const customers: ICustomersModel = await CustomersService.insert(req.body);
...
export async function remove(req: RequestWithUser, res: Response, next: NextFunction): Promise<void> {
    try {
        const customers: ICustomersModel = await CustomersService.remove(req.params.id);
